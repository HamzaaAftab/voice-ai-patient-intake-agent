...
Voice AI / Conversational AI Engineer
...
| Requirement Details Phone Number Provision a real, dialable U.S. phone number using a telephony provider (e.g., Twilio, Vonage, Vapi, Retell, Bland.ai). Voice Interaction The agent must conduct a natural, conversational flow — not a rigid IVR menu. It should feel like speaking with a human intake coordinator. LLM-Powered Use any LLM (OpenAI, Anthropic, Google, open-source). Must understand varied phrasing, ask clarifying questions, and handle corrections. Confirmation Before saving, the agent must read back all collected information and ask the caller to confirm or correct any field. Error Handling If the caller provides invalid data (e.g., a 3-digit phone number, a future date of birth), the agent |
| --- |
| must re-prompt specifically for that field. |


| Requirement | Details |
| --- | --- |
| Call Completion | After successful registration, provide a brief confirmation (e.g., "You're all set, [First Name].") |
|   | and end gracefully. |
...
Conversational Note: The agent does NOT need to ask for every optional field on every call. It should collect required fields, then offer: "I can also collect your insurance information, emergency contact, and preferred language. Would you like to provide any of those?" — letting the caller opt in.
...
| Persistence | Data must survive server restarts. If we register "Jane Doe" on Call 1, she must exist when we |
|   | query on Call 2. |
...
| --- | --- | --- |
| GET | /patients | List all patients. Support optional query params: ?last_name=, ?date_of_birth=, |
|   |   | ?phone_number= |
...
| Area | Expectation |
| --- | --- |
| Deployment | The system must be running and callable at the time of review. We will call the number. Use any |
|   | hosting you prefer (Railway, Render, Fly.io, Replit, ngrok, AWS, GCP, etc.). |
| Code Quality | Clean, readable, well-organized code. We don't expect perfection in 3 hours, but we do expect |
|   | intentional structure. |
| README | A clear README.md with: setup instructions, architecture description, tech stack justification, env |
|   | variables required, known limitations or trade-offs. |
| Security | Do not hardcode API keys in source code. Use environment variables. Basic input sanitization on |
|   | the API. |
...
Is the voice agent actually good?
...
Are the right choices being made?
...
Could another engineer pick this up?
...
Tip: Platforms like Vapi and Retell abstract much of the telephony/STT/TTS complexity and let you focus on the LLM prompt, tool definitions, and backend. This is the fastest path to a working system within 3 hours.
...
Good luck. We look forward to calling your agent.
